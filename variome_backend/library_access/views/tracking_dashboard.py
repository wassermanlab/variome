...
class DashboardForm(forms.Form):
    ...
    user = forms.ModelMultipleChoiceField(
        queryset=LibraryUser.objects.all(), required=False
    )
...
def tracking_dashboard(request):
    ...
    target_variant = Variant.objects.filter(
        variant_id=form.cleaned_data["variant"]
    ).first()
    target_variant_id = target_variant.id if target_variant else None
    variant_filter = (
        {"url__endswith": f"variant/{target_variant_id}"}
        if target_variant_id
        else {"url__contains": "api/variant"}
    )
    ...
    selected_users = (
        ...
        else LibraryUser.objects.all()
    )
    user_stats = [u for u in user_stats if u in selected_users]

    user_details = []
    for user in user_stats:
        UserHitsInTimeFrame = Pageview.objects.filter(
            visitor__user=user, view_time__gte=start_time
        )
        ...
        user_details.append(
            {
                ...
                "time_on_site": str(user.time_on_site.seconds // 60)
                + ":"
                + str(user.time_on_site.seconds % 60),
            }
        )
    ...
    for variant_url in variant_urls:
        ...
        if variant and variant.variant_id not in variant_logging:
            user_list = (
                Pageview.objects.filter(url=variant_url["url"])
                .values(
                ...
                .order_by("visitor__user__email")
                .distinct("visitor__user__email")
            )
            users = [
                {
                    "get_full_name": f"{u['visitor__user__first_name']} {u['visitor__user__last_name']}",
                    "email": u["visitor__user__email"],
                    "username": u["visitor__user__username"],
                }
                for u in user_list
            ]
    ...
    for pageview in Pageview.objects.filter(
        ...
        **variant_filter,
        visitor__user__in=selected_users,
    ).order_by("-view_time"):
